...
for(let i = 1 ; i < arr.length ; i++) {
    if (arr[i].Rating_of_the_place >= 4){
        document.getElementById(`tab${i}`).style.backgroundColor = 'rgb(61, 190, 61)';}
        if (arr[i].Rating_of_the_place<2){
...
for(let i=0; i<arr.length; i++) {
    ...
    document.getElementById(`tab${y.Id-1}`).style.backgroundColor= `blue`;
      }
      console.log(better);
      let y =arr.find(x => x.Price==3000);
      $(y).remove();
